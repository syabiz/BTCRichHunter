...
def _tpu_worker(counter, result_q, targets: frozenset, stop_ev,
    ...
    rng = jr.PRNGKey(secrets.randbits(32))   # Fix: 64-bit overflow on C long → use 32-bit
    ...
    while not stop_ev.is_set():
        ...
        for i in range(batch_sz):
            ...
            if use_range:
                # Fix: ki adalah integer 256-bit dari 32 byte acak.
                # Petakan ke dalam span dengan modulo agar selalu valid dalam range.
...
if __name__ == "__main__":
    # Fix: JAX is multithreaded — os.fork() causes deadlock warning on Linux.
    # 'spawn' creates fresh interpreter per worker, safe with JAX/TPU.
    try:
