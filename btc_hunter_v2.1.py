#!/usr/bin/env python3
# ==============================================================================
#  Bitcoin Rich Hunter  —  MAXIMUM PERFORMANCE EDITION v2.1
#  ─────────────────────────────────────────────────────────────────────────────
#
#  PENINGKATAN TERBARU:
#  1. Dukungan multi‑GPU – pengguna dapat memilih jumlah GPU yang digunakan
#  2. Deteksi daftar GPU beserta nama dan ID-nya
#  3. CLI argument --gpus untuk menentukan jumlah GPU secara non‑interaktif
#  4. Setiap GPU worker berjalan pada perangkat yang berbeda
#  5. Mode hybrid GPU + CPU tetap dipertahankan
#  6. Penyesuaian tampilan status untuk menunjukkan jumlah GPU dan CPU aktif
#
#  Fitur lain sama seperti v2.0:
#    - Telegram otomatis, monitor chat ID tersemat
#    - P2TR (Taproot) taptweak BIP341 compliant
#    - Rejection sampling untuk range mapping
#    - Target frozenset via mmap (sharing)
#    - coincurve batch pubkey (lebih cepat)
#    - Dukungan NVIDIA, AMD, Intel, Apple, TPU, CPU
# ==============================================================================
...
# ── Bitcoin Puzzle ranges (unsolved as of 2026) ───────────────────────────────
PUZZLE_RANGES = [
    ...
    ("#160 — 160-bit", "0x8000000000000000000000000000000000000000", "0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF", 160),
    ("Custom — Enter your own range", None, None, 0),
    ("Full  — The entire range of secp256k1 (random)",  None, None, -1),
]
...
# ==============================================================================
#  ACCELERATOR DETECTION (MULTI‑GPU SUPPORT)
# ==============================================================================

ACCEL_TYPE    = "CPU"
ACCEL_NAME    = "CPU"
ACCEL_BACKEND = None
ACCEL_DEVICES = []          # list of (device_id, device_name) for GPUs
ACCEL_DEVICE_COUNT = 0
...
def _try_cupy():
    try:
        ...
        for i in range(count):
            ...
            name = props.get("name", b"NVIDIA GPU").decode() if isinstance(props.get("name"), bytes) else str(props.get("name", f"GPU {i}"))
            devices.append((i, name))
        # Use device 0 to test
...
def _try_torch(device: str):
    try:
        ...
        if device == "cuda":
            ...
            devices = [(i, torch.cuda.get_device_name(i)) for i in range(count)]
        ...
        if device == "xpu":
            ...
            devices = [(i, torch.xpu.get_device_name(i)) for i in range(count)]
            return torch, devices
    except Exception:
        pass
    return None, []
...
def _try_smi():
    ...
    if out:
        names = out.split("\n")
        return [(i, names[i]) for i in range(len(names)) if names[i]]
    try:
        with open("/proc/driver/nvidia/version") as f:
            return [(0, "NVIDIA GPU (" + f.read().split("\n")[0][:30] + ")")]
    except Exception:
        return []
...
def _try_rocm():
    ...
    if not has_rocm:
        return None, []
    t, devs = _try_torch("cuda")
    if t and devs:
        # filter AMD GPUs by name?
        return t, devs
    ...
    for i, line in enumerate(lines):
        ...
        if m:
            devices.append((i, m.group(1).strip()))
    if not devices:
        devices = [(0, "AMD GPU")]
    return None, devices
...
def _try_jax_tpu():
    ...
    try:
        ...
        if devs:
            ...
            # For TPU, we consider it as one "device" with multiple cores
            return jax, [(0, f"TPU {n} ×{len(devs)}")]
    except Exception:
        pass
    if is_tpu_env:
...
def detect_accel():
    global ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND, ACCEL_DEVICES, ACCEL_DEVICE_COUNT
    print(f"\n{Fore.CYAN}🔍 Detecting accelerator...{Style.RESET_ALL}")

    # TPU
    j, devs = _try_jax_tpu()
    if j and devs:
        ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "TPU_JAX", devs[0][1], j
        ACCEL_DEVICES = devs
        ACCEL_DEVICE_COUNT = len(devs)
        print(f"  {Fore.GREEN}✅ {ACCEL_NAME}  [TPU/JAX]{Style.RESET_ALL}")
        if ACCEL_DEVICE_COUNT > 1:
            print(f"     {ACCEL_DEVICE_COUNT} TPU cores available (will be used automatically)")
        return

    # NVIDIA — CuPy
    cp, devs = _try_cupy()
    if cp and devs:
        ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_NVIDIA", "NVIDIA GPU", ("cupy", "cuda")
        ACCEL_DEVICES = devs
        ACCEL_DEVICE_COUNT = len(devs)
        print(f"  {Fore.GREEN}✅ {ACCEL_DEVICE_COUNT} NVIDIA GPU(s) detected via CuPy{Style.RESET_ALL}")
        for i, name in devs:
            print(f"     GPU {i}: {name}")
        return
    ...
    smi_devs = _try_smi()
    if smi_devs:
        print(f"  {Fore.YELLOW}⚠  {len(smi_devs)} NVIDIA GPU(s) detected — attempting CuPy install{Style.RESET_ALL}")
        cv = _cuda_ver()
        if cv:
            cp2, devs2 = _install_cupy(cv)
            if cp2 and devs2:
                ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_NVIDIA", "NVIDIA GPU", ("cupy", "cuda")
                ACCEL_DEVICES = devs2
                ACCEL_DEVICE_COUNT = len(devs2)
                print(f"  {Fore.GREEN}✅ {ACCEL_DEVICE_COUNT} NVIDIA GPU(s) ready [CuPy auto-install]{Style.RESET_ALL}")
                return
        t, devs = _try_torch("cuda")
        if t and devs:
            ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_NVIDIA", "NVIDIA GPU", ("torch", "cuda")
            ACCEL_DEVICES = devs
            ACCEL_DEVICE_COUNT = len(devs)
            print(f"  {Fore.GREEN}✅ {ACCEL_DEVICE_COUNT} NVIDIA GPU(s) via PyTorch{Style.RESET_ALL}")
            return
    ...
    t, devs = _try_rocm()
    if t and devs:
        ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_AMD", "AMD GPU", ("torch", "cuda")
        ACCEL_DEVICES = devs
        ACCEL_DEVICE_COUNT = len(devs)
        print(f"  {Fore.GREEN}✅ {ACCEL_DEVICE_COUNT} AMD GPU(s) detected via ROCm{Style.RESET_ALL}")
        for i, name in devs:
            print(f"     GPU {i}: {name}")
        return
    elif devs:
        print(f"  {Fore.YELLOW}⚠  AMD GPU(s) detected — attempting ROCm install{Style.RESET_ALL}")
        t2, devs2 = _install_rocm()
        if t2 and devs2:
            ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_AMD", "AMD GPU", ("torch", "cuda")
            ACCEL_DEVICES = devs2
            ACCEL_DEVICE_COUNT = len(devs2)
            print(f"  {Fore.GREEN}✅ {ACCEL_DEVICE_COUNT} AMD GPU(s) ready [ROCm auto-install]{Style.RESET_ALL}")
            return

    # Intel
    t, devs = _try_torch("xpu")
    if t and devs:
        ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_INTEL", "Intel GPU", ("torch", "xpu")
        ACCEL_DEVICES = devs
        ACCEL_DEVICE_COUNT = len(devs)
        print(f"  {Fore.GREEN}✅ {ACCEL_DEVICE_COUNT} Intel GPU(s) detected{Style.RESET_ALL}")
        for i, name in devs:
            print(f"     GPU {i}: {name}")
        return

    # Apple MPS
    t, devs = _try_torch("mps")
    if t and devs:
        ACCEL_TYPE, ACCEL_NAME, ACCEL_BACKEND = "GPU_APPLE", devs[0][1], ("torch", "mps")
        ACCEL_DEVICES = devs
        ACCEL_DEVICE_COUNT = len(devs)
        print(f"  {Fore.GREEN}✅ {ACCEL_NAME}  [Apple/MPS]{Style.RESET_ALL}")
        return
...
# ==============================================================================
#  WORKERS (with multi‑GPU support)
# ==============================================================================
...
def _gpu_worker(worker_id: int, device_id: int, counter, result_q, targets: frozenset,
                stop_ev, backend: str, device: str, batch_sz: int, key_range: tuple):
    """
    GPU worker for a specific device. Uses double-buffering.
    ...
    # ── Setup random generator on the specified device ────────────────────────
    if backend == "cupy":
        try:
            import cupy as cp
            ...
            print(f"\n{Fore.RED}[GPU {device_id}] CuPy init failed: {e} → falling back to CPU{Style.RESET_ALL}")
            _cpu_worker(worker_id, counter, result_q, targets, stop_ev, 1000, key_range)
            return
    else:  # torch (cuda, xpu, mps)
        try:
            import torch
            if device.startswith("xpu"):
                import intel_extension_for_pytorch  # noqa
            dev_obj = torch.device(f"{device}:{device_id}")
            # test
            ...
            print(f"\n{Fore.RED}[GPU {device_id}] torch/{device} init failed: {e} → falling back to CPU{Style.RESET_ALL}")
            _cpu_worker(worker_id, counter, result_q, targets, stop_ev, 1000, key_range)
            return

    print(f"  {Fore.GREEN}✅ GPU worker {device_id} active [{backend}/{device}:{device_id}] batch={batch_sz}{Style.RESET_ALL}")
    ...
    def _prefetch(idx):
        try:
            ...
            print(f"\n{Fore.RED}[GPU {device_id}] Prefetch error: {e}{Style.RESET_ALL}")
            gen_err[0] = True
...
def _tpu_worker(counter, result_q, targets: frozenset, stop_ev,
    ...
    rng = jr.PRNGKey(secrets.randbits(32))
...
def display_status(counter, t0: float, mode: str, num_gpu_workers: int, num_cpu_workers: int,
    ...
    while not stop_ev.is_set():
        ...
        sys.stdout.write(
            f"\r[{Fore.CYAN}{mode}{Style.RESET_ALL}|GPU:{Fore.GREEN}{num_gpu_workers}{Style.RESET_ALL} "
            f"CPU:{Fore.GREEN}{num_cpu_workers}{Style.RESET_ALL}] "
            f"{rng_label}"
...
def print_banner(targets_count: int, target_files: list):
    ...
    # Show GPU count if available
    gpu_info = ""
    if ACCEL_DEVICE_COUNT > 0 and ACCEL_TYPE != "CPU" and ACCEL_TYPE != "TPU_JAX":
        gpu_info = f" ({ACCEL_DEVICE_COUNT} GPU{'s' if ACCEL_DEVICE_COUNT>1 else ''})"

    print(f"""
{Fore.CYAN}╔══════════════════════════════════════════════════════════════════════╗
║    BITCOIN RICH HUNTER  ·  MAXIMUM PERFORMANCE EDITION  v2.1        ║
║  Multi‑GPU · TPU · CPU · Telegram · .gz Support                      ║
╚══════════════════════════════════════════════════════════════════════╝{Style.RESET_ALL}

  Accelerator  : {c}{ACCEL_NAME}{gpu_info}  [{l}]{Style.RESET_ALL}
...
def select_cores(cli_cores: str = None) -> int:
    total = multiprocessing.cpu_count()
    if cli_cores:
        if cli_cores.lower() == "all":
            return total
        try:
            return min(max(1, int(cli_cores)), total)
        except ValueError:
            pass
...
def select_gpus(cli_gpus: str = None) -> int:
    """Return number of GPUs to use (0 means use all available)."""
    if ACCEL_DEVICE_COUNT == 0:
        return 0
    if cli_gpus:
        if cli_gpus.lower() == "all":
            return ACCEL_DEVICE_COUNT
        try:
            val = int(cli_gpus)
            if 0 <= val <= ACCEL_DEVICE_COUNT:
                return val
            print(f"{Fore.RED}Invalid --gpus value. Must be between 0 and {ACCEL_DEVICE_COUNT}.{Style.RESET_ALL}")
            sys.exit(1)
        except ValueError:
            print(f"{Fore.RED}Invalid --gpus value. Use integer or 'all'.{Style.RESET_ALL}")
            sys.exit(1)

    # Interactive selection
    print(f"\n{Fore.YELLOW}Select number of GPUs to use:{Style.RESET_ALL}")
    opts = list(range(1, ACCEL_DEVICE_COUNT + 1)) + [0]
    for i, c in enumerate(opts, 1):
        tag = f"  {Fore.CYAN}← all{Style.RESET_ALL}" if c == ACCEL_DEVICE_COUNT else ""
        tag0 = f"  {Fore.WHITE}← none (CPU only){Style.RESET_ALL}" if c == 0 else ""
        print(f"  {Fore.CYAN}[{i}]{Style.RESET_ALL} {c} GPU(s){tag}{tag0}")
    while True:
        try:
            idx = int(input(f"\n{Fore.WHITE}Choice (1-{len(opts)}): {Style.RESET_ALL}").strip()) - 1
            if 0 <= idx < len(opts):
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Bitcoin Rich Hunter — Maximum Performance Edition v2.1",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python bitcoin_hunter.py                          # interactive mode
  python bitcoin_hunter.py --cores 8               # 8 CPU cores
  python bitcoin_hunter.py --gpus 2                # use 2 GPUs (if available)
  python bitcoin_hunter.py --range 0x400...:0x7FF...  # specific hex range
  python bitcoin_hunter.py --range full             # full secp256k1 range
  python bitcoin_hunter.py --cores 4 --gpus all     # all GPUs + 4 CPU
  python bitcoin_hunter.py --no-telegram            # disable Telegram
  python bitcoin_hunter.py btc.txt --cores all      # custom target file
    ...
    p.add_argument("--gpus", type=str, default=None,
                   help="Number of GPUs to use (integer or 'all')")
...
def main():
    ...
    # Determine GPU count
    if not cpu_only and ACCEL_DEVICE_COUNT > 0:
        num_gpu_workers = select_gpus(args.gpus)
        if num_gpu_workers == 0:
            cpu_only = True
            print(f"\n{Fore.YELLOW}No GPU selected, falling back to CPU.{Style.RESET_ALL}")
        else:
            print(f"\n{Fore.GREEN}✅ Using {num_gpu_workers} GPU worker(s){Style.RESET_ALL}")

    # CPU cores
    if cpu_only or num_gpu_workers == 0:
        # pure CPU mode
        num_cpu_workers = select_cores(args.cores)
        mode_str = f"CPU×{num_cpu_workers}"
        print(f"\n{Fore.GREEN}✅ Using {num_cpu_workers} CPU core(s){Style.RESET_ALL}\n")
    else:
        # hybrid mode: GPU + extra CPU
        extra_cpu = args.extra_cpu
        if extra_cpu == 0 and args.cores is None:
            try:
                ans = input(f"{Fore.CYAN}Add extra CPU workers in hybrid mode? (y/n): {Style.RESET_ALL}").strip().lower()
                if ans == "y":
                    extra_cpu = select_cores(args.cores)
            except EOFError:
                pass
        elif args.cores:
            extra_cpu = select_cores(args.cores)
        num_cpu_workers = extra_cpu
        mode_str = f"{ACCEL_TYPE}×{num_gpu_workers}" + (f"+CPU×{num_cpu_workers}" if num_cpu_workers else "")
        print()
    ...
    # ── Banner (updated with worker counts) ───────────────────────────────────
    ...
    def start_workers():
        ps = []
        # GPU workers
        if not cpu_only and num_gpu_workers > 0:
            # assign devices in order
            for i in range(num_gpu_workers):
                device_id = ACCEL_DEVICES[i][0]  # actual device index
                if ACCEL_TYPE == "TPU_JAX":
                    # TPU uses a single worker with multiple cores
                    p = Process(target=_tpu_worker,
                                args=(counter, found_q, targets, stop_ev, gpu_batch, key_range))
                    ps.append(p)
                    break  # only one TPU worker
                else:
                    bk, dev = ACCEL_BACKEND
                    p = Process(target=_gpu_worker,
                                args=(i, device_id, counter, found_q, targets, stop_ev,
                                      bk, dev, gpu_batch, key_range))
                    ps.append(p)
    ...
    def launch_status(n_gpu, n_cpu):
        threading.Thread(
            target=display_status,
            args=(counter, t0, mode_str, n_gpu, n_cpu, stop_ev, key_range),
            daemon=True
        ).start()

    try:
        procs = start_workers()
        launch_status(num_gpu_workers, num_cpu_workers)
